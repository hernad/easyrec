...
/**
 * Strategy for calculating similarities between items using an adjusted cosine measure. <p/> Works just like the cosine
 * measure but subtracts the average rating of the item from each vector component. <p/> See [Sarwar et al, 2001]. <p/>
 ...
 */
public class AdjustedCosineSimilarityCalculationStrategy extends AbstractSimilarityCalculationStrategy {
    // --------------------------- CONSTRUCTORS ---------------------------

    public AdjustedCosineSimilarityCalculationStrategy() {
        super();
    }
    ...
    public String getSourceInfo() {
        return ItemItemService.SOURCE_INFO_COSINE_ADJUSTED;
    }
    ...
    protected double getAverage1(final RatedTogether<Integer, Integer> ratedTogether,
        ...
        return averageRatings.get(ratedTogether.getRating1().getUser()).getRatingValue();
    }
    ...
    protected Map<Integer, RatingVO<Integer, Integer>> getAverageItemRatings(final Integer tenantId,
        ...
        final Map<Integer, RatingVO<Integer, Integer>> averageItemRatingsPerUser = new HashMap<Integer, RatingVO<Integer, Integer>>(
                averageItemRatings.size());

        for (final RatingVO<Integer, Integer> rating : averageItemRatings)
            averageItemRatingsPerUser.put(rating.getUser(), rating);

        return averageItemRatingsPerUser;
    }
}
